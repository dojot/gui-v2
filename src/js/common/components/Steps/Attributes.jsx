...
const Attributes = (props) => {
  ...
  const handleSubmit = (values) => {
    handleClick({ type: 'next', payload: { values: values.attributes, key: 'attributes' } })
  }
  ...
  return (
    ...
    >
      {(formikProps) => <AttributesForm {...formikProps} {...otherProps} onBack={handleBack} />}
    </Formik>
  )
}
...
const AttributesForm = (props) => {
  ...
  const handleToggle = (
    ...
    const currentIndex = checked.map((item) => item.attributeID).indexOf(attributeID)
    ...
    <Fragment key={meta.id}>
      ...
      >
        ...
        <Tooltip title={meta.id} placement="bottom-start">
          <ListItemText id={labelId} primary={`[${meta.label}] ${value.label}`} />
        </Tooltip>
