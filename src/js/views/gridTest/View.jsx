...
class ShowcaseLayout extends Component {
  ...
  onUpdateLayout = (layout) => {
    console.log(layout)
    this.setState({ layout: staticLayout });
  }

  createElement = (element, index, onDelete, onPin) => {
  ...
  createNewWidget = () => {
    ...
    console.log('vou adicionar um novo widget')
    console.log(layout)
    this.setState({
      layout: _.concat(layout, {
        i: uuidv4(),
        x: (layout.length % 2) * 6,
        y: Infinity,
        w: 6,
  ...
  onRemoveItem = (i) => {
    const { layout } = this.state;
    this.setState({ layout: _.reject(layout, { i }) });
  }
  ...
  render() {
    const { layout, compactType } = this.state;
    const { cols, rowHeight, className } = this.props;
    return (
      <div>
        <Button size="small" variant="outlined" color="primary" onClick={() => this.createNewWidget()}>adicionar</Button>
        <ResponsiveReactGridLayout
          cols={cols}
          rowHeight={rowHeight}
          className={className}
          ...
          onResizeStop={this.onUpdateLayout}
          onDragStop={this.onUpdateLayout}
          measureBeforeMount={false}
          compactType={compactType}
          verticalCompact
          preventCollision={!compactType}
        >
          {_.map(layout, (element, index) => this.createElement(
            element,
            index,
            this.onRemoveItem,
            this.onPin
